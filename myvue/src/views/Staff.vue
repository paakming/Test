    ...
    <div>
      ...
      <el-dialog title="添加" :visible.sync="dialogFormVisibleAdd" width="500px"  :close-on-click-modal="false" :before-close="clearDialog">
        <el-form :model="formAdd" ref='formAdd'>
          ...
          <el-form-item label="部门" :label-width="formLabelWidth">
            <el-input v-model="formAdd.department" autocomplete="off"></el-input>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm('确认删除？',{
        ...
        this.request.delete('/doctor/'+row.jobid).then((res)=>{
          if (res.code==='200'){
            this.$message({
              message:'删除成功',
              type:"success"
    ...
    DeleteIds(){
      let ids = this.multipleSelection.map(v => v.jobid)
    ...
    edit(){
      this.request.put('/doctor',this.form).then((res)=>{
        this.dialogFormVisible = false
        if (res.code==='200'){
          this.$message({
            message:'编辑成功',
